# Módulo "personal" — coach de execução de exercícios

Aba 🏋 PERSONAL do Jarvis. Envie um vídeo curto de um exercício e veja cada
repetição analisada; ou monte a rotina da semana; ou consulte a biblioteca de
23 exercícios.

## Onde o código mora

**Não aqui.** O código-fonte é o projeto `D:\8 - Claude - projeto\Personal`.
Neste repositório existem só:

| arquivo | o que é |
|---|---|
| `personal-modulo.js` | a ponte com a antena — escrita à mão |
| `personal/` | o build, **gerado**. Não edite nada aqui dentro. |

Para atualizar depois de mexer no Personal:
...
implementação — **se um dia aparecer uma rota `/api/personal` aqui, é sinal de
...
| rota | o que faz |
|---|---|
| `GET /personal/*` | a aplicação construída, com fallback de SPA |

## O servidor de arquivos é compartilhado

`modulo-estatico.js` serve tanto o personal quanto o receita. Foi extraído por um
